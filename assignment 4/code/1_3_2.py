...
def load_and_preprocess_data(csv_file_path):
    ...
    # Filter rows where 'Protocol' is 'ICMP'
    filtered_df = df[df['Protocol'].isin(['ICMP'])]

    return filtered_df

def plot_packet_count(filtered_df, interval=1):
    # Group data by time interval and calculate the packet count
...
def plot_packet_length(filtered_df, interval=1):
    # Group data by time interval and calculate the packet length
...
if __name__ == "__main__":
    ...
    plot_packet_count(filtered_data, interval=1)
    plot_packet_length(filtered_data, interval=1)
